...
class ResponseForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'statut': forms.Select(attrs={'class': 'border p-2 rounded w-full'}),
            'date_reponse': forms.DateInput(attrs={'type': 'date', 'class': 'border p-2 rounded w-full'}),
            'temps_reponse': forms.NumberInput(attrs={'class': 'border p-2 rounded w-full'}),
            'commentaires': forms.Textarea(attrs={'class': 'border p-2 rounded w-full', 'rows': 4}),
            'response_file': forms.FileInput(attrs={'class': 'border p-2 rounded w-full'}),
        }
    ...
    def __init__(self, *args, lettre=None, **kwargs):
        ...
        self.fields['temps_reponse'].required = False  # Will be calculated
        self.fields['response_file'].required = True

    def clean_response_file(self):
        response_file = self.cleaned_data.get('response_file')
        if not response_file:
            raise forms.ValidationError("Un fichier de réponse est requis.")
        if self.lettre:
            ...
            valid_extensions = {
                ...
                'PDF': ['.pdf'],
                'Word': ['.doc', '.docx'],
                'Autre': None  # Allow any extension for 'Autre'
            }
