...
def dashboard_Reponse(request):
    ...
    # Filter responses by destination and role
    responses = Response.objects.filter(destination=destination)
    if profile.role == 'saisie_er':
        responses = responses.filter(lettre__service=profile.service)  # Restrict to specific service for saisie_er
    ...
    # Create lettres_with_responses list
...
def submit_response(request, lettre_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response_time = (datetime.now().date() - reception_date).days * 24
            response.temps_reponse = math.floor(response_time)
            response.save()
            logger.info("Response saved for lettre: %s, destination: %s", lettre.id, destination.nom)
            ...
            responded_count = lettre.reponses.filter(statut='repondu', user__isnull=True).count()
            logger.debug("Lettre: %s, Expected: %s, Responded: %s", lettre.id, expected_response_count, responded_count)
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'success': False, 'message': 'Erreur dans le formulaire.', 'errors': form.errors.as_json()}, status=400)
            for field, errors in form.errors.items():
                for error in errors:
                    messages.error(request, f"{field}: {error}")
    else:
